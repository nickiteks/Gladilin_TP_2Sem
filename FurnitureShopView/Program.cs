            ...
            if (type.BaseType == typeof(BaseViewModel))
            {
                ...
                foreach (var conf in config)
                {
                    ...
                    if (prop != null)
                    {
                        ...
                        var attributes = prop.GetCustomAttributes(typeof(FurnitureShopBusinessLogic.Attributes.ColumnAttribute), true);
                        if (attributes != null && attributes.Length > 0)
                        {
                            foreach (var attr in attributes)
                            {
                                // ищем нужный нам атрибут
                                if (attr is FurnitureShopBusinessLogic.Attributes.ColumnAttribute columnAttr)
                                {
