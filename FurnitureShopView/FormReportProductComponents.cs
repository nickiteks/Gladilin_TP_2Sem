    ...
    public partial class FormReportProductComponents : Form
    {
        ...
        private void buttonSaveToExcel_Click(object sender, EventArgs e)
        {
            using (var dialog = new SaveFileDialog { Filter = "xlsx|*.xlsx" })
            {
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    try
                    {
                        logic.SaveFurnitureComponentsToExcelFile(new ReportBindingModel
                        {
        ...
        private void FormReportProductComponents_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    if (!dates.Contains(order.Key.Date))
                    {
                        dates.Add(order.Key.Date);
                    }
                }
                ...
                if (dict != null)
                {
                    ...
                    foreach (var date in dates)
                    {
                        ...
                        foreach (var order in dict.Where(rec => rec.Key.Date == date.Date))
                        {
                            dataGridView.Rows.Add(new object[] { "", order.Key });
                        }
                        dataGridView.Rows.Add(new object[]
                        {
                            "General Sum:", "", GenSum
