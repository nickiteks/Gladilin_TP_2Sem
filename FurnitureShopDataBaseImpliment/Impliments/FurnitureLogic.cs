...
namespace FurnitureShopDataBaseImpliment.Impliments
{
    public class FurnitureLogic : IFurnitureLogic
    {
            ...
            using (var context = new FurnitureShopDataBase())
            {
                using (var transaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (model.Id.HasValue)
                        {
                            var productComponents = context.FurnitureComponents.Where(rec => rec.FurnitureId == model.Id.Value).ToList();
                            // удалили те, которых нет в модели
                            context.FurnitureComponents.RemoveRange(productComponents.Where(rec => !model.FurnitureComponents.ContainsKey(rec.ComponentId)).ToList());
                            context.SaveChanges();
            ...
            using (var context = new FurnitureShopDataBase())
            {
                using (var transaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        context.FurnitureComponents.RemoveRange(context.FurnitureComponents.Where(rec => rec.FurnitureId == model.Id));
        ...
        public List<FurnitureViewModel> Read(FurnitureBindingModel model)
        {
            using (var context = new FurnitureShopDataBase())
            {
                return context.Furnitures.Where(rec => model == null || rec.Id == model.Id)
                .ToList()
                .Select(rec => new FurnitureViewModel
                {
                    ...
                    Price = rec.Price,
                    FurnitureComponents = context.FurnitureComponents.Include(recPC => recPC.Component)
